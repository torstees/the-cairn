...
async def test_seed_sets_creates_set(db: AsyncSession) -> None:
    ...
    records = [{"title": "Morning Set", "description": None, "source": None,
                "abc_header": None, "flow_difficulty": None,
...
async def test_seed_sets_stores_members_in_order(db: AsyncSession) -> None:
    ...
    records = [{"title": "Two Tune Set", "members": [
        {"tune_title": "Tune A", "setting_label": None},
        {"tune_title": "Tune B", "setting_label": None},
    ]}]
    await seed_sets(db, records)
    members = (await db.execute(
        select(TuneSetMember)
        .join(TuneSet)
        .where(TuneSet.title == "Two Tune Set")
        .order_by(TuneSetMember.order)
    )).scalars().all()
...
async def test_seed_sets_warns_on_missing_tune(db: AsyncSession, capsys) -> None:
    records = [{"title": "Broken Set", "members": [
        {"tune_title": "Nonexistent Tune", "setting_label": None}
    ]}]
    loaded, skipped, errors = await seed_sets(db, records)
    assert loaded == 1
    assert errors == 0
...
async def test_seed_sets_stores_metadata(db: AsyncSession) -> None:
    records = [{"title": "Meta Set", "description": "Great set", "source": "Session",
                "abc_header": "P:AB", "flow_difficulty": 4,
                "flow_difficulty_notes": "Tricky transition", "members": []}]
    await seed_sets(db, records)
